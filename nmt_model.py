...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder =nn.LSTM(input_size=embed_size, hidden_size=hidden_size, bidirecetional=True, batch_first=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        @param source_padded (Tensor): Tensor of padded source sentences with shape (src_len, b), where
                                       ...
                                       these have already been sorted in order of longest to shortest sentence.   is index
        ...
        # [max_len,batch_size] -> [maxlen, batch_size,emb_size]
        X = self.model_embeddings.source(source_padded)  # Embed the source sentences
        
        # 
        packed_X = pack_padded_sequence(X, source_lengths) 
        enc_hiddens, (last_hidden, last_cell) = self.encoder(packed_X)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)  # Pad the packed sequence
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 2. Construct tensor Y of target sentences using target model embeddings
        Y = self.model_embeddings.target(target_padded)
    
        # 3. Loop over the time dimension of Y
        for Y_t in torch.split(Y, 1, dim=0):
            Y_t = Y_t.squeeze(dim=0)  # (b, e)
            Ybar_t = torch.cat([Y_t, o_prev], dim=1)  # Concatenate Y_t and o_prev
        ...
        # 4. Convert combined_outputs from list of tensors to a single tensor
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ### YOUR CODE HERE (~3 Lines)
        # 1. Apply the decoder LSTM cell to Ybar_t and dec_state to obtain the new dec_state
        # 注意这里可以融合上个时间点的输出信息作为当前时间点的输入
        dec_state = self.decoder(Ybar_t, dec_state)
        
        # 2. Split dec_state into its two parts (dec_hidden, dec_cell)
        dec_hidden, dec_cell = dec_state
        
        # 3. Compute attention scores e_t
        ...
        # 5. Compute tensor O_t by applying Tanh function and dropout
